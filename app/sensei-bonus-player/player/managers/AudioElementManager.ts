...
class AudioElementManager extends PlayerEventEmitter {
  ...
  get isReverse(): boolean {
    return !!(
      this.audioElement && this.audioElement.src === this.reversedAudioBlobUrl
    );
  }
  ...
  private attachAudioElementEvents() {
    ...
    this.audioElement?.addEventListener(
      "timeupdate",
      this.onAudioElementTimeupdate
    );

    return () => {
      this.audioElement?.removeEventListener(
        "playing",
        this.onAudioElementPlaying
      );
      this.audioElement?.removeEventListener(
        "pause",
        this.onAudioElementPaused
      );
  ...
  private onScrubbed = () => {
    ...
    if (this.audioElement.paused) {
      this.log.info(
        "storing current position after scrub",
  ...
  private onSeeked = () => {
    ...
    this.log.info(
      "seeked with playback rate",
      this.audioContextManager.playbackRate
    );
    this.audioElement.currentTime = this.audioContextManager.currentTime;
    this.audioElement.playbackRate = Math.abs(
      this.audioContextManager.playbackRate
    );
  };
  ...
  private onTimeupdate = (position: number) => {
    if (!this.audioElement || this.isBackgrounded || this.audioElement.paused)
      return;
    // Sync any >0.5s lag between audio element and audio context every 10 seconds
    if (
      position - this.previousPosition >
      10 * Math.abs(this.audioContextManager.playbackRate)
    ) {
      this.previousPosition = position;
      const diff = Math.abs(this.audioElement.currentTime - position);
      this.log.info(
        "checking for the lag between audio element and context",
        diff
      );
      if (diff > 0.5) {
        this.log.info(
          `detected a difference of ${diff} between context and the audio element: resetting to`,
          position
        );
  ...
  private onAudioElementEnded = () => {
    if (this.isBackgrounded && this.audioElement) {
      this.emit(
        PLAYER_EVENTS.ended,
        this.audioElement.src === this.reversedAudioBlobUrl
          ? "reverse"
          : "forward"
      );
    }
  ...
  seekTo = ({ seekTime }: { seekTime?: number }) => {
    ...
    this.emit(PLAYER_EVENTS.seeking, {
      ...
      direction: this.isReverse ? "reverse" : "forward",
    });
  ...
  initialize = (url: string) => {
    ...
    return new Promise<void>(async (resolve, reject) => {
      if (!this.forwardAudioBlobUrl || !this.silentAudioBlobUrl) {
        throw new Error(
          "Attempted to initialize without defined audio blob urls"
        );
      }
      ...
      this.audioElement.src = this.isBackgrounded
        ? this.forwardAudioBlobUrl
        : this.silentAudioBlobUrl;
      this.audioElement.load();
  ...
  private onVisibilityChange = async () => {
    ...
    this.log.info(
      `visibility has changed to the ${this.isBackgrounded ? "background" : "foreground"}`
    );
    ...
    if (isHidden && this.forwardAudioBlobUrl && this.reversedAudioBlobUrl) {
      ...
      this.audioElement.src = this.audioContextManager.isReverse
        ? this.reversedAudioBlobUrl
        : this.forwardAudioBlobUrl;
      this.audioElement.currentTime = this.audioContextManager.isReverse
        ? this.audioContextManager.duration -
          this.audioContextManager.currentTime
        : this.audioContextManager.currentTime;
      this.audioElement.playbackRate = this.audioContextManager.isSeeking
        ? 1
        : Math.abs(this.audioContextManager.playbackRate);
      this.log.info(
        `setting audio element position to ${this.audioElement.currentTime} and playback rate to ${this.audioElement.playbackRate}`
      );
      ...
      this.log.info(
        "restoring audio context and silent audio to play in the foreground"
      );
      const shouldPlayAudioElement =
        !this.audioElement.pause && !this.isReverse;
      ...
      this.log.info(
        `Setting audio element currentTime to ${this.audioElement.currentTime} and playback rate to ${this.audioElement.playbackRate}`
      );
      try {
        const shouldResume =
          !this.audioElement.paused && this.audioContextManager.isPlaying;
        await this.audioContextManager.onForeground(shouldResume);
        this.audioContextManager.playbackRate = this.audioContextManager
          .isReverse
          ? this.audioElement.playbackRate * -1
          : this.audioElement.playbackRate;
        this.log.info(
          `setting audio context position to ${this.audioContextManager.currentTime} and playback rate to ${this.audioContextManager.playbackRate}`
        );
        if (shouldPlayAudioElement) {
          this.play();
        }
  ...
  private onTouchStart = () => {
    if (
      this.audioContextManager.state !== PlayerState.PLAYING ||
      !this.audioElement
    ) {
  ...
  private attachSingleTouchEventListener() {
    this.log.info(
      "listening for next touch event to play silent audio element"
    );
