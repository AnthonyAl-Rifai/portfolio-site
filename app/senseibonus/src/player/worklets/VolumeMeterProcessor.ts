...
export class VolumeMeterProcessor extends AudioWorkletProcessor {
  ...
  process(inputs: Float32Array[][]) {
    if (inputs[0].length === 2) {
      ...
      this._volumeRight = Math.max(rmsRight, this._volumeRight * SMOOTHING_FACTOR);

      // Post a message to the node every 16ms
      if (currentTime - this._lastUpdate > FRAME_INTERVAL) {
        this.port.postMessage([this._volumeLeft * VOLUME_PEAK, this._volumeRight * VOLUME_PEAK]);
        this._lastUpdate = currentTime;
      }
    } else if (inputs[0][0]) {
      ...
      if (currentTime - this._lastUpdate > FRAME_INTERVAL) {
        ...
        this.port.postMessage([this._volumeLeft * VOLUME_PEAK, this._volumeLeft * VOLUME_PEAK]);
        this._lastUpdate = currentTime;
      }
    }
